...
if [ ! -f "$ARQ_CORE" ]; then
    echo "❌ Erro: Arquivo '$ARQ_CORE' não encontrado!"
    exit 1
fi

if ! ls saidas/*.c &>/dev/null; then
    echo "Nenhum arquivo .c encontrado."
    echo "Você rodou 'make test' primeiro?"
    exit 0
fi

for arquivo_c in $(ls -v "$PASTA_SAIDAS"/*.c)
do
  ...
  executavel="$PASTA_SAIDAS/exec/$base_name"
   ...
   if gcc -I. -o "$executavel" "$ARQ_CORE" "$arquivo_c" &> /dev/null; then
